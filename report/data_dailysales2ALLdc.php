...
<body>
	...
	order  by P_Code asc ";
		...
		{	$ByPro[]=$By['P_Code'];
			echo '<th align="center" colspan="3">'.$By['PRODUCTNAME'].'</th>';
			 $countProduct++;
		}
	...
	{ 	//echo "<br>**".$reUDC['UID'];
		$pp++;echo '<tr class="mousechange" height="30" valign= "top" >
			...
			for($i=0;$i<$countProduct;$i++)
			{	$Pro=$ByPro[$i];
				$sqlSum1="select sum(d.st_unit_qty_3)/con1.st_unit_qty as box
				,(sum(d.st_unit_qty_3)%con1.st_unit_qty)/con2.st_unit_qty as pack
				,sum(d.totalamount) as totalamount
				from st_Sale_detail d left join st_Sale_head
				on d.Sales_Docno = st_Sale_head.Sales_Docno LEFT OUTER JOIN  dbo.st_item_unit_con con1
				ON d.P_Code = con1.P_Code AND con1.st_unit_id = 'ลัง' LEFT OUTER JOIN  dbo.st_item_unit_con con2
				ON d.P_Code = con2.P_Code AND con2.st_unit_id = 'แพ็ค'
				
				where  d.Createby='$reUDC[UID]' 
				and cast(st_Sale_head.Sales_Docdate  as date) = '$dateTo'  
				and d.P_Code='$Pro'  
				
				group by con1.st_unit_qty,con2.st_unit_qty ";
				
				//left join st_CN_detail CNd on d.Sales_Docno = CNd.Ref_Docno and d.P_Code=CNd.P_Code  and CNd.Ref_Docno is null
			...
			<? 	
				...
				$sqlTotalby['SumBy']=$sqlTotalby['SumBy']-$sqlCN2['SumByGroup'];
			...
			<td align="center ">
			<? //ผลต่าง
			$new=$sqlTotalby['SumBy']-$sqlTotalPay['sumPay'];
	...
	<tr class="mousechange" >
			...
			?>
			<td ><?=number_format($SUM_totalamount,2); $SUM_totalamount=0;?> </td>
			<td >-<?=number_format($TotalCNBy,2); $TotalCNBy=0;?> </td>
